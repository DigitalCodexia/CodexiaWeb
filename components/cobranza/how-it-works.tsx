      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="mx-auto max-w-2xl text-center mb-20">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-accent/10 border border-accent/20 mb-6">
            <span className="text-sm font-medium text-accent">{p.howEyebrow}</span>
          </div>
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold tracking-tight text-foreground">
            {p.howHeading}{" "}
            <span className="bg-gradient-to-r from-accent to-primary bg-clip-text text-transparent">
              {p.howHeadingHighlight}
            </span>
          </h2>
        </div>
        ...
        <div className="relative max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-6">
            {p.steps.map((step, index) => {
              const color = index === 1 ? "accent" : "primary"
              return (
                <div key={index} className="relative group">
                  <div className={`relative rounded-3xl p-8 border transition-all duration-500 hover:shadow-2xl ${
                    ...
                    <div className={`absolute bottom-0 right-0 w-20 h-20 rounded-tl-3xl ${color === "primary" ? "bg-primary/5" : "bg-accent/5"}`} />
                  </div>
