  ...
  return (
    <section className="relative pt-28 pb-16 lg:pt-36 lg:pb-24 overflow-hidden">
      <div className="absolute inset-0 -z-10">
        ...
        <div className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(ellipse_80%_50%_at_50%_-20%,oklch(0.75_0.15_260/0.3),transparent)]" />
        <div className="absolute bottom-0 right-0 w-full h-full bg-[radial-gradient(ellipse_60%_40%_at_80%_100%,oklch(0.75_0.15_180/0.2),transparent)]" />
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        <div className="mx-auto max-w-4xl text-center">
          <div ref={badgeRef} className="inline-flex items-center gap-3 px-5 py-2.5 rounded-full bg-gradient-to-r from-primary/10 via-card to-accent/10 border border-primary/20 mb-10 shadow-lg shadow-primary/5">
            ...
            <span className="text-sm font-medium text-foreground">Gestion profesional de cobranza</span>
          </div>

          <h1 ref={headingRef} className="text-4xl sm:text-5xl lg:text-7xl font-bold tracking-tight text-foreground leading-[1.1]">
            <span className="text-balance">Tus cuentas por cobrar,</span>
            <br />
          ...
          <p ref={subRef} className="mt-8 text-lg lg:text-xl text-muted-foreground max-w-2xl mx-auto text-pretty leading-relaxed">
            Recordatorios programados, reportes detallados y seguimiento completo de cada cuenta.
            Simplifica tu cobranza y mejora tu flujo de efectivo.
          </p>

          <div ref={ctaRef} className="mt-12 flex flex-col sm:flex-row items-center justify-center gap-4">
            <Button size="lg" className="w-full sm:w-auto text-base px-8 h-14 rounded-xl shadow-lg shadow-primary/25 bg-gradient-to-r from-primary to-primary/90 hover:shadow-xl hover:shadow-primary/30 transition-all" asChild>
          ...
          <div ref={tagsRef} className="mt-14 flex flex-wrap items-center justify-center gap-x-8 gap-y-4">
            {[
              { label: "Configuracion rapida", color: "accent" },
              { label: "Datos seguros", color: "primary" },
              { label: "Soporte en espanol", color: "accent" },
            ].map((item) => (
            ...
            <div className="rounded-2xl bg-card/95 backdrop-blur border border-border p-4 shadow-2xl shadow-primary/10 animate-bounce">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">Empresa ABC - $2,500</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div ref={floatRightRef} className="absolute -right-4 top-1/3 hidden lg:block z-20">
            <div className="rounded-2xl bg-card/95 backdrop-blur border border-border p-4 shadow-2xl shadow-primary/10 animate-bounce" style={{ animationDelay: "0.5s" }}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-foreground">Recordatorio enviado</p>
                  <p className="text-xs text-muted-foreground">3 clientes notificados</p>
                </div>
              </div>
            </div>
          </div>
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
      <div className="rounded-2xl bg-gradient-to-br from-primary/10 via-card to-primary/5 p-5 border border-primary/20">
        <p className="text-sm text-muted-foreground mb-3">Por cobrar</p>
        <p className="text-3xl font-bold text-foreground">$45,230</p>
        <p className="mt-2 text-sm text-accent">+12% este mes</p>
      </div>
      <div className="rounded-2xl bg-gradient-to-br from-accent/10 via-card to-accent/5 p-5 border border-accent/20">
        <p className="text-sm text-muted-foreground mb-3">Recordatorios</p>
        <p className="text-3xl font-bold text-foreground">156</p>
        <p className="mt-2 text-sm text-muted-foreground">Enviados esta semana</p>
      </div>
      <div className="rounded-2xl bg-gradient-to-br from-secondary via-card to-secondary/30 p-5 border border-border">
        <p className="text-sm text-muted-foreground mb-3">Clientes activos</p>
        <p className="text-3xl font-bold text-foreground">48</p>
        <p className="mt-2 text-sm text-muted-foreground">Con cuentas pendientes</p>
      </div>
      <div className="md:col-span-2 rounded-2xl bg-card p-5 border border-border">
        ...
        <div className="space-y-3">
          {[
            { client: "Empresa ABC", amount: "$2,500", status: "Pagado", color: "accent" },
            { client: "Tech Solutions", amount: "$4,800", status: "Pendiente", color: "muted" },
            { client: "Comercial XYZ", amount: "$1,200", status: "Enviado", color: "primary" },
          ].map((item, i) => (
            <div key={i} className="flex items-center justify-between py-2.5 border-b border-border/50 last:border-0">
              ...
              <div className="flex items-center gap-3">
                ...
                <span className={`text-xs px-2.5 py-1 rounded-full font-medium ${
                  item.status === "Pagado" ? "bg-accent/15 text-accent" :
                  item.status === "Pendiente" ? "bg-muted text-muted-foreground" :
      ...
      <div className="rounded-2xl bg-gradient-to-br from-primary/5 to-accent/5 p-5 border border-border">
        <p className="text-sm font-semibold text-foreground mb-4">Cobros del mes</p>
